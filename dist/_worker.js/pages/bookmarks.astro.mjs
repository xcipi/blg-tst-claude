...
const $$Bookmarks = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const allTags = bookmarks.length > 0 ? [...new Set(bookmarks.flatMap((b) => b.tags))] : [];
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Bookmarky - sec.blog" }, { "default": async ($$result2) => renderTemplate` ${renderComponent($$result2, "NavBar", $$NavBar, { "currentPath": Astro2.url.pathname })} ${maybeRenderHead()}<main class="min-h-screen pt-24 pb-16 px-4"> <div class="max-w-7xl mx-auto"> <div class="mb-8"> <div class="text-emerald-500 font-mono text-sm mb-4">$ cat ~/bookmarks</div> <h1 class="text-4xl font-bold text-white mb-2">Bookmarky</h1> <p class="text-gray-400">Zaujímavé odkazy z internetu</p> </div> ${error ? renderTemplate`<div class="bg-red-900/20 border border-red-500 rounded-lg p-4 text-center"> <p class="text-red-400 font-mono">${error}</p> <p class="text-gray-400 text-sm mt-2">Databáza nie je dostupná. Spusti: wrangler d1 create sec-blog-db</p> </div>` : bookmarks.length === 0 ? renderTemplate`<div class="text-center py-16"> <p class="text-gray-400 font-mono text-lg">Zatiaľ žiadne bookmarky.</p> <p class="text-gray-500 text-sm mt-2">Pridaj prvý bookmark cez admin panel.</p> </div>` : renderTemplate`${renderComponent($$result2, "Fragment", Fragment, {}, { "default": async ($$result3) => renderTemplate`<div class="mb-8 space-y-4">  <div class="flex flex-wrap gap-2"> <button data-privacy="all" class="privacy-filter px-3 py-1 rounded-full text-xs font-mono border transition-all bg-emerald-500 text-black border-emerald-500 font-bold active">
Všetky
</button> <button data-privacy="public" class="privacy-filter px-3 py-1 rounded-full text-xs font-mono border transition-all bg-gray-900 text-emerald-500 border-gray-800 hover:border-emerald-500">
Verejné
</button> <button data-privacy="private" class="privacy-filter px-3 py-1 rounded-full text-xs font-mono border transition-all bg-gray-900 text-emerald-500 border-gray-800 hover:border-emerald-500">
Súkromné 🔒
</button> </div>  ${allTags.length > 0 && renderTemplate`<div class="overflow-x-auto pb-2"> <div class="flex gap-2 min-w-max"> <button data-tag="all" class="tag-filter px-3 py-1 rounded-full text-xs font-mono border transition-all bg-emerald-500 text-black border-emerald-500 font-bold active">
all
</button> ${allTags.map((tag) => renderTemplate`<button${addAttribute(tag, "data-tag")} class="tag-filter px-3 py-1 rounded-full text-xs font-mono border transition-all bg-gray-900 text-emerald-500 border-gray-800 hover:border-emerald-500"> ${tag} </button>`)} </div> </div>`} </div> <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4" id="bookmarks-grid"> ${bookmarks.map((bookmark) => renderTemplate`<div class="bookmark-item"${addAttribute(bookmark.tags.join(","), "data-tags")}${addAttribute(bookmark.isPrivate ? "private" : "public", "data-privacy")}> ${renderComponent($$result3, "BookmarkCard", $$BookmarkCard, { "title": bookmark.title, "url": bookmark.url, "tags": bookmark.tags, "date": bookmark.date, "isPrivate": bookmark.isPrivate })} </div>`)} </div> ` })}`} </div> </main> ${renderComponent($$result2, "Footer", $$Footer, {})} ` })} `;
